...
fun BluetoothGatt.printGattTable() {
    ...
    services.forEach { service ->
        val characteristicsTable = service.characteristics.joinToString(separator = "\n|--", prefix = "|--",
            transform = fun(char: BluetoothGattCharacteristic): CharSequence {
                var description = "${char.uuid}: ${char.printProperties()}"
                if (char.descriptors.isNotEmpty()) {
                    description += "\n" + char.descriptors.joinToString(
                        separator = "\n|------",
                        prefix = "|------",
                        transform = fun(descriptor: BluetoothGattDescriptor): CharSequence {
                            return "${descriptor.uuid}: ${descriptor.printProperties()}"
                        }
                    )
                }
                return description
...
fun BluetoothGattCharacteristic.printProperties(): String = mutableListOf<String>().apply {
    if (isReadable()) add("PODACI O KORIŠTENJU")
    if (isWritable()) add("ODREĐIVANJE BRZINE")
...
fun BluetoothGattDescriptor.printProperties(): String = mutableListOf<String>().apply {
    if (isReadable()) add("ČITANJE")
    if (isWritable()) add("PISANJE")
    if (isEmpty()) add("EMPTY")
}.joinToString()
...
fun ByteArray.toAsciiString() : String = toString(Charsets.US_ASCII)
